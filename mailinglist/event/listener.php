	...
	*/
	public function send_to_mailinglist($event)
	{
			 ...
			 $notification_type_name == 'notification.type.post'))
		{
			...
			// retrieve post user email and flagged listener email from config -- wjang
			$username = $post_data['post_username'];
			$sql = 'SELECT user_email FROM ' . USERS_TABLE . " WHERE username_clean = '" . $this->db->sql_escape(utf8_clean_string($username)) . "'";
			$result = $this->db->sql_query($sql);
			$member = $this->db->sql_fetchrow($result);
			$this->db->sql_freeresult($result);
			$this->dumpy($member['user_email']);
			$this->dumpy($this->config['mailinglist_listeneremail']);
			
			// if post user email is flagged listener email, then return and do not send email to mailing list -- wjang
			if ($member['user_email'] == $this->config['mailinglist_listeneremail']) {
				$this->dumpy('Post is from listeneremail account. Aborting send_message().');
				return;
			}
